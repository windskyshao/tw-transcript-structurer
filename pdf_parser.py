...
class UnifiedTranscriptParser:
    ...
    def _extract_building_layers_corrected_order(self, text: str) -> OrderedDict:
        ...
        # 🔥 步驟3.5：層次區塊內的「名稱：面積」附加項
        #    例：層次寫成「外廊：２２．５１平方公尺；門廊：４．７９平方公尺」
        #    這些是主建物層次面積的一部分（會計入標示部「總面積」），但格式與一般層次不同
        #    （全形冒號＋全形數字、無 *** 補位），步驟3(名稱 空格 數字)與步驟4(名稱須以「面積」結尾)
        #    兩邊都抓不到 → 主建物坪數會少算（實測少 27.30㎡＝8.26坪）。
        #    ⚠ 僅在「層次區塊」內比對（層 次： 到 建築完成日期/附屬建物用途/其他登記事項/共有部分 之前），
        #      避免誤抓附屬建物、共有部分(公設)、總面積而把不屬於主建物的面積算進來。
        layer_block_match = re.search(
            r'層\s*次[:：](.*?)(?=建築完成日期|附屬建物用途|其他登記事項|共有部分|$)',
            text, re.S)
        if layer_block_match:
            block = layer_block_match.group(1)
            # 名稱限「連續中文2~6字」，故「；」不會被吃進名稱；值不含 * 故帶 *** 的官方欄位不會誤中
            for _name, _val in re.findall(
                    r'([一-鿿]{2,6})[:：]\s*([\d\.\,０-９．，]+)\s*平方公尺',
                    block):
                # 以「面積」結尾的名稱（總面積/層次面積/停車空間面積…）一律留給步驟4處理，
                # 兩邊職責切開，避免同一項被重複加成兩筆而灌水
                if _name.endswith('面積'):
                    continue
                _val_h = self.fullwidth_to_halfwidth(_val)
                layer_data[f"層次{layer_index}"] = _name
                layer_data[f"層次面積{layer_index}"] = f"{_val_h}平方公尺"
                layer_index += 1
                print(f"      ✅ 提取層次附加項: {_name} = {_val_h}平方公尺")
...
def extract_comprehensive_ocr_data(images: List[Tuple[str, np.ndarray]], ocr_mgr: TranscriptOCRManager) -> Tuple[List[Dict[str, str]], List[str]]:
    ...
    # 通用欄位模式（兼容一類和二類）
    field_patterns = {
        # 🔥 修正：姓名後面接「統一編號」或「字串結尾」都要能抓到。
        #    OCR 常把姓名與統一編號切成不同圖片，姓名那張後面沒有「統一編號」，
        #    原本只用 (?=\s*統一編號) 的前瞻會失敗，導致所有權人整欄空白。
        "所有權人": r"所有權人[:：]\s*(.+?)(?=\s*統一編號|\s*$)",
        "統一編號": r"統一編號[:：]\s*([^\s\n]+)",
        "出生日期": r"出生日[期日][:：]\s*([^\s\n]+)",
        "住址": r"住\s*址[:：]\s*([^\n]+)",
        "地址": r"地\s*址[:：]\s*([^\n]+)",
